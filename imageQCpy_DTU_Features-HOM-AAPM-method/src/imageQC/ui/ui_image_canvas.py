...
class ImageCanvas(GenericImageCanvas):
    ...
    def add_contours_to_all_rois(self, colors=None, reset_contours=True,
        ...
        for color_no, roi_no in enumerate(roi_indexes):
            # Fejlhåndtering: Sørg for, at roi_no er gyldigt
            if roi_no < 0 or roi_no >= len(this_roi):
                print(f"ROI index out of range: roi_no={roi_no}, len(this_roi)={len(this_roi)}")
                continue  # Skip til næste iteration hvis index er uden for rækkevidde

            # Sørg for, at ROI ikke er tom
            if this_roi[roi_no] is None:
                print(f"Empty ROI for roi_no={roi_no}")
                continue
       ...
       # Tjek om modaliteten er Xray, og om hom_tab_alt == 4 for AAPM
        if self.main.current_modality == 'Xray' and self.main.current_paramset.hom_tab_alt == 4:
           # Hent beregnede ROIs fra input_main (eller self.main)
            ...
            if roi_array is not None and len(roi_array) > 0:
               # Antallet af ROIs
