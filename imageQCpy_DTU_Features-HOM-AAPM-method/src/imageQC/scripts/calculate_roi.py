...
def get_rois(image, image_number, input_main):
    ...
    # Valider ROI-størrelsen matcher billedstørrelsen
    if roi_array is not None:
        if isinstance(roi_array, list):
            for i, roi in enumerate(roi_array):
                if roi.shape != img_shape:
                    errmsg = f"ROI shape mismatch for array {i}: expected {img_shape}, got {roi.shape}"
                    print(errmsg)  # Du kan også raise en Exception i stedet for at printe fejlen
                    # Du kan skalere eller håndtere fejlen her
        else:
            if roi_array.shape != img_shape:
                errmsg = f"ROI shape mismatch: expected {img_shape}, got {roi_array.shape}"
                print(errmsg)  # Du kan også raise en Exception i stedet for at printe fejlen
                # Du kan skalere ROI eller returnere en fejl
...
def get_roi_hom_aapm(image_info, paramset):
    ...
    # Opret en tom maske, der har samme størrelse som billedet
    combined_mask = np.zeros(image_info.shape, dtype=bool)
    ...
    for row in range(rows):  # Gå kun gennem gyldige rækker
        for col in range(cols):  # Gå kun gennem gyldige kolonner
            ...
            try:
                ...
                # Placer ROI i den kombinerede maske
                combined_mask[int(start_y):int(end_y), int(start_x):int(end_x)] = roi
