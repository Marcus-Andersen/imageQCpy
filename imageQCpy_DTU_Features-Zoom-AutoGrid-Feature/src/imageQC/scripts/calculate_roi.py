...
def calculate_aapm_rois(image_info, paramset, call_id=None):
    ...
    try:
        ...
        zoom_center_x = getattr(paramset, 'zoom_center_x', 0)  # sætter center til (0,0) som udgangspunkt
        zoom_center_y = getattr(paramset, 'zoom_center_y', 0)
        
        print(f"[calculate_aapm_rois] Zoom center ({zoom_center_x}, {zoom_center_y}) for call ID {call_id}")

        # Test med en ROI i midten af billedet
        roi_test = get_roi_rectangle(image_info.shape, 50, 50, (image_info.shape[1] // 2, image_info.shape[0] // 2))
        print(f"Test ROI Signal: {np.count_nonzero(roi_test)}")
        ...
        start_x = int(- (zoomed_width / (2 * image_info.pix[0])))
        start_y = int(- (zoomed_height / (2 * image_info.pix[1])))
        slut_x = int((zoomed_width / (2 * image_info.pix[0])))
        slut_y = int((zoomed_height / (2 * image_info.pix[1])))

        print(f"[DEBUG] Før begrænsning, start_x={start_x}, start_y={start_y}")
